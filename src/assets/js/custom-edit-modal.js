...
async function showIngredientDetail(ingredientId) {
  console.log("Show ingredient detail called with ID:", ingredientId);
  try {
    document.querySelectorAll('.ingredient-detail-modal').forEach(modal => {
      modal.remove();
    });
    ...
    // Create modal content
    const modal = document.createElement('div');
    modal.classList.add('fixed', 'inset-0', 'bg-black', 'bg-opacity-50', 'z-50', 'flex', 'items-center', 'justify-center', 'ingredient-detail-modal');
    
    const modalContent = `
      <div class="bg-white rounded-lg w-full max-w-3xl mx-4 overflow-hidden">
        <div class="p-4 bg-gray-50 flex justify-between items-center">
          <h3 class="text-lg font-medium">${ingredient.name || 'Ingredient Details'}</h3>
          <button class="close-modal-btn text-gray-400 hover:text-gray-600">
            <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
            </svg>
        ...
        <div class="p-6">
          ${ingredient.ingredient_image_url ? `<img src="${ingredient.ingredient_image_url}" alt="${ingredient.name}" class="w-full h-64 object-cover rounded-md mb-4">` : ''}
          
          <div class="grid grid-cols-2 gap-4 mb-4 text-sm">
            <div class="flex flex-col p-2 border rounded-md">
              <span class="font-medium">Quantity</span>
              <span>${ingredient.quantity || 'Not specified'} ${ingredient.unit || ''}</span>
            </div>
            <div class="flex flex-col p-2 border rounded-md">
              <span class="font-medium">Price</span>
              <span>$${parseFloat(ingredient.price || 0).toFixed(2)}</span>
            </div>
          </div>
          
          <div class="mb-4">
            <h4 class="text-md font-medium mb-2">Notes</h4>
            <p class="text-gray-700">${ingredient.notes || 'No notes provided.'}</p>
          </div>
          
          <div class="mb-4">
            <h4 class="text-md font-medium mb-2">Description</h4>
            <p class="text-gray-700">${ingredient.ingredient_description || 'No description provided.'}</p>
          </div>
          
          <div class="flex justify-between text-sm text-gray-500">
            <span>Created: ${ingredient.created_at ? new Date(ingredient.created_at).toLocaleDateString() : 'Unknown'}</span>
          </div>
        </div>
        <div class="p-4 bg-gray-50 flex justify-end gap-2 border-t border-gray-200">
          <button class="edit-btn py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-primary hover:bg-primary-700">
            Edit
          </button>
        </div>
      </div>
    ...
    editBtn.addEventListener('click', () => {
      document.body.removeChild(modal);
      editIngredientDetail(ingredientId);
    });
    
  } catch (error) {
    console.error('Error showing ingredient details:', error);
  }
}

async function editIngredientDetail(ingredientId) {
  console.log("Edit ingredient detail called with ID:", ingredientId);
  try {
    document.querySelectorAll('.ingredient-detail-modal').forEach(modal => {
      modal.remove();
    });
    ...
    const modalContent = `
      <div class="bg-white rounded-lg w-full max-w-3xl mx-4 overflow-hidden">
        <div class="p-4 bg-gray-50 flex justify-between items-center">
          <h3 class="text-lg font-medium">Edit Ingredient</h3>
          <button class="close-modal-btn text-gray-400 hover:text-gray-600">
            <svg class="h-6 w-6" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
            </svg>
        ...
        <form id="edit-ingredient-form" class="p-6">
          ...
          <div class="mb-4">
            ...
            <input type="text" id="name" name="name" class="w-full rounded-md border border-gray-300 p-2" value="${ingredient.name || ''}">
          </div>
          
          <div class="mb-4">
            <label for="ingredient_image_url" class="block text-sm font-medium text-gray-700 mb-1">Image URL</label>
            <input type="text" id="ingredient_image_url" name="ingredient_image_url" class="w-full rounded-md border border-gray-300 p-2" value="${ingredient.ingredient_image_url || ''}">
            ${ingredient.ingredient_image_url ? `<img src="${ingredient.ingredient_image_url}" alt="${ingredient.name}" class="mt-2 w-full h-32 object-cover rounded-md">` : ''}
          </div>
          
          <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
              <label for="quantity" class="block text-sm font-medium text-gray-700 mb-1">Quantity</label>
              <input type="text" id="quantity" name="quantity" class="w-full rounded-md border border-gray-300 p-2" value="${ingredient.quantity || ''}">
            </div>
            
            <div>
              <label for="unit" class="block text-sm font-medium text-gray-700 mb-1">Unit</label>
              <input type="text" id="unit" name="unit" class="w-full rounded-md border border-gray-300 p-2" value="${ingredient.unit || ''}">
            </div>
          </div>
          
          <div class="mb-4">
            <label for="price" class="block text-sm font-medium text-gray-700 mb-1">Price ($)</label>
            <input type="number" id="price" name="price" step="0.01" class="w-full rounded-md border border-gray-300 p-2" value="${ingredient.price || '0.00'}">
          </div>
          
          <div class="mb-4">
            <label for="notes" class="block text-sm font-medium text-gray-700 mb-1">Notes</label>
            <textarea id="notes" name="notes" rows="2" class="w-full rounded-md border border-gray-300 p-2">${ingredient.notes || ''}</textarea>
          </div>
          
          <div class="mb-4">
            <label for="ingredient_description" class="block text-sm font-medium text-gray-700 mb-1">Description</label>
            <textarea id="ingredient_description" name="ingredient_description" rows="4" class="w-full rounded-md border border-gray-300 p-2">${ingredient.ingredient_description || ''}</textarea>
          </div>
    ...
    // Handle form submission
    const form = modal.querySelector('#edit-ingredient-form');
    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      ...
      try {
        ...
        notification.innerHTML = `
          <div class="flex items-center">
            <div class="py-1"><svg class="fill-current h-6 w-6 text-green-500 mr-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M2.93 17.07A10 10 0 1 1 17.07 2.93 10 10 0 0 1 2.93 17.07zm12.73-1.41A8 8 0 1 0 4.34 4.34a8 8 0 0 0 11.32 11.32zM6.7 9.29L9 11.6l4.3-4.3 1.4 1.42L9 14.4l-3.7-3.7 1.4-1.42z"/></svg></div>
            <div>
              <p class="text-sm">Ingredient updated successfully!</p>
            </div>
          </div>
        `;
        document.body.appendChild(notification);
    ...
    console.error('Error showing ingredient edit form:', error);
  }
}
